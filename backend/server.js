...
const startDatabase = async () => {
  try {
    ...
    if (process.env.MONGO_URL || process.env.MONGODB_URI) {
      // Production: gunakan MongoDB dari Railway atau Atlas
      mongoUri = process.env.MONGO_URL || process.env.MONGODB_URI;
      console.log('🌐 Menggunakan MongoDB dari environment variable...');
    } else {
      // Development: gunakan MongoDB in-memory
      console.log('💾 Menggunakan MongoDB In-Memory (mode development)...');
    ...
    console.error('❌ Gagal menyalakan database:', error);
    process.exit(1);
  }
};
...
// 2. READ: Mendapatkan detail satu produk (publik)
